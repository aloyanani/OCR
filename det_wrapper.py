...
class DetWrapper:
    ...
    def predict(
        ...
        if return_heatmap:
            ...
            score_link_heatmap = image_utils.cvt2heatmap_image(score_link)

            # Save heatmap automatically
            os.makedirs("heatmaps", exist_ok=True)
            existing = len(os.listdir("heatmaps"))
            heatmap_file = os.path.join("heatmaps", f"heatmap_{existing + 1}.png")
            cv2.imwrite(heatmap_file, cv2.cvtColor(heatmap, cv2.COLOR_RGB2BGR))

            # Optional visualization
            plt.figure(figsize=(10, 4))
            plt.subplot(1, 2, 1)
            plt.imshow(heatmap)
            plt.title("Text Heatmap")
            plt.axis("off")

            plt.subplot(1, 2, 2)
            plt.imshow(score_link_heatmap)
            plt.title("Link Heatmap")
            plt.axis("off")
            plt.close()  # close figure to avoid GUI popups

        if return_heatmap:
            return boxes, heatmap_file
        else:
            return boxes
